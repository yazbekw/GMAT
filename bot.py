...
"""
بوت أسئلة شامل - نسخة محسنة ومتطورة
- دعم ملفات JSON مستقلة لكل قسم (Critical Reasoning, Data Sufficiency, Problem Solving)
- أزرار رجوع في كل مكان
- اختبار مخصص مع نتائج وأزرار للعودة
- وضع التعلم، الإشارات، الأخطاء، الإحصائيات، وغيرها
"""
...
QUESTIONS_DIR = os.path.join(BASE_DIR, "questions")  # مجلد يحتوي على ملفات JSON منفصلة
ADMIN_IDS = list(map(int, os.getenv("ADMIN_IDS", "").split(","))) if os.getenv("ADMIN_IDS") else []
TOKEN = os.getenv("BOT_TOKEN")
if not TOKEN:
    raise ValueError("BOT_TOKEN not set in environment")

# التأكد من وجود مجلد الأسئلة
...
def parse_options(options_raw: Union[dict, list]) -> List[str]:
    """تحويل الخيارات من صيغ مختلفة إلى مصفوفة."""
    if isinstance(options_raw, dict):
        # ترتيب المفاتيح أبجدياً (A, B, C, ... أو 1, 2, 3, ...)
...
def parse_answer(answer_raw, options_count: int) -> Optional[int]:
    """تحويل الإجابة من صيغ مختلفة إلى مؤشر رقمي."""
...
# ============================================================
# تحميل الأسئلة من ملفات JSON مستقلة
# ============================================================
def load_questions_from_files():
    """تحميل الأسئلة من جميع ملفات JSON في مجلد questions."""
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("DELETE FROM questions")  # مسح البيانات القديمة

    json_files = list(Path(QUESTIONS_DIR).glob("*.json"))
    if not json_files:
        logger.warning(f"لا توجد ملفات JSON في {QUESTIONS_DIR}")
        return 0
    ...
    for file_path in json_files:
        ...
        # استخراج قائمة الأسئلة من الهيكل
        questions_list = []
        ...
        if not questions_list:
            logger.warning(f"لم يتم العثور على أسئلة في {file_path}")
            continue
...
def build_result_buttons():
    """أزرار تظهر بعد انتهاء الاختبار."""
...
async def exit_to_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """الخروج من أي وضع والعودة للقائمة الرئيسية."""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    await query.edit_message_text(
...
# ============================================================
# ConversationHandlers للإدارة (اختصاراً)
# ============================================================
...
def main():
    ...
    # معالجات الأزرار الأساسية (ترتيبها مهم)
    ...
    application.add_handler(CallbackQueryHandler(exit_to_menu, pattern="^exit_menu$"))
